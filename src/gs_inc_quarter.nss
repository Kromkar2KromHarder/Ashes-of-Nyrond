...
void gsQUCreateKey(object oQuarter, object oTarget)
{
    object oContainer = GetLocalObject(GetModule(), "GS_TEMPORARY");
    ...
    if (GetIsObjectValid(oContainer))
    {
        ...
        if (sUniqueID != "")
        {
            object oKey = CreateItemOnObject(GS_QU_TEMPLATE_KEY, oContainer);

            if (GetIsObjectValid(oKey))
            {
                CopyObject(oKey, GetLocation(oTarget), oTarget, sUniqueID);
                DestroyObject(oKey);
            }
        }
    }

...
//return TRUE if caller can obtain allies
int gsCBTalentObtainAllies();
...
int gsCBTalentDragonsBreath(object oTarget);
...
int gsCBGetIsPerceived(object oTarget, object oObject = OBJECT_SELF)
{
    ...
    if (GetActionMode(oTarget, ACTION_MODE_STEALTH))         return FALSE;
...
object gsCBGetAttackTarget(object oObject = OBJECT_SELF, object oTarget = OBJECT_INVALID)
{
    if (! GetIsObjectValid(oTarget) ||
        ...
        GetFactionEqual(oObject, oTarget) ||
                                 ...
                                 int nCounterMeasure    = FALSE)
{
    ...
    while (GetIsObjectValid(oObject))
    {
        if (! GetIsDead(oObject) &&
            (nFriendly && (GetIsReactionTypeFriendly(oObject) || GetIsFriend(oObject)))  ||
            (nNeutral  && (GetIsReactionTypeNeutral(oObject)  || GetIsNeutral(oObject))) ||
            (nHostile  && (GetIsReactionTypeHostile(oObject)  || GetIsEnemy(oObject))))
        {
...
void gsCBDetermineCombatRound(object oTarget = OBJECT_INVALID)
{
    ...
    if (Random(100) >= nAttack)
    {
        ...
        if (Random(100) >= 25 && gsCBTalentObtainAllies())         return;
    }

    //spell
    if (GetObjectSeen(oTarget) && Random(100) < nSpell)
    {
        ...
        if (Random(100) >= 50 && gsCBTalentDragonsBreath(oTarget)) return;
...
int gsCBCastSpellAtObject(int nSpell, object oTarget = OBJECT_SELF)
{
    if (gsC3VerifySpell(nSpell, oTarget))
    {
        ...
        ActionDoCommand(gsC2SetResistSpell(nSpell, oTarget));
        return TRUE;
    }
...
int gsCBTalentObtainAllies()
{
    ...
    talent tTalent   = GetCreatureTalentBest(TALENT_CATEGORY_BENEFICIAL_OBTAIN_ALLIES, 100);
    if (! GetIsTalentValid(tTalent))                             return FALSE;

    location lSource = GetLocation(OBJECT_SELF);
    vector vSource   = GetPositionFromLocation(lSource);
    object oTarget   = gsCBGetCreatureAtLocation(lSource, FALSE, FALSE, TRUE, 0.0, 30.0);
    if (! GetIsObjectValid(oTarget))                             return FALSE;

    vector vDirection = GetPosition(oTarget) - vSource;
    float fDistance   = VectorMagnitude(vDirection) / 1.5;
    vector vTarget    = VectorNormalize(vDirection) * fDistance + vSource;
    lSource           = Location(GetArea(OBJECT_SELF), vTarget, VectorToAngle(vDirection));

    ActionUseTalentAtLocation(tTalent, lSource);
...
int gsCBTalentDragonsBreath(object oTarget)
{
    ...
    if (GetIsTalentValid(tTalent) &&
        GetIdFromTalent(tTalent) == nLastTalentID)
    {
    ...
    {
        SetLocalInt(OBJECT_SELF, "GS_CB_TALENT_BREATH", GetIdFromTalent(tTalent));
        return TRUE;
    }

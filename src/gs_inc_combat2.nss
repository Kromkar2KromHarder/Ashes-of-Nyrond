...
const string GS_C2_RESIST_SPELL               = "GS_C2_RS_";
...
//return FALSE if there is a high chance that oCreature will resist nSpell
int gsC2GetResistSpell(int nSpell, object oCreature = OBJECT_SELF);
...
//set resistance of oCreature against nSpell cast by oCaster
void gsC2SetResistSpell(int nSpell, object oCreature, object oCaster = OBJECT_SELF);
...
int gsC2GetResistSpell(int nSpell, object oCreature = OBJECT_SELF)
{
    return Random(11) < GetLocalInt(oCreature, GS_C2_RESIST_SPELL + IntToString(nSpell));
}
//----------------------------------------------------------------
struct gsC2Effect gsC2GetEffect(object oCreature = OBJECT_SELF)
{
...
void gsC2SetResistSpell(int nSpell, object oCreature, object oCaster = OBJECT_SELF)
{
    string sString   = GS_C2_RESIST_SPELL + IntToString(nSpell);
    int nResistSpell = GetLocalInt(oCreature, sString);

    switch (ResistSpell(oCaster, oCreature))
    {
    case 1:
    case 2:
    case 3:
        if (++nResistSpell > 10) nResistSpell = 10;
        SetLocalInt(oCreature, sString, nResistSpell);
        break;

    default:
        DeleteLocalInt(oCreature, sString);
        break;
    }
}

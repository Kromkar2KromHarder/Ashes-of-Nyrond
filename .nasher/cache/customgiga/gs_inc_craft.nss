...
const int GS_CR_MODIFIER_DC           = 250;
...
void gsCRCreateProduktion(struct gsCRRecipe stRecipe, object oPC, object oContainer = OBJECT_SELF);
...
//produce oItem in oContainer by oPC
void gsCRProduce(object oItem, object oPC, object oContainer = OBJECT_SELF);
//internally used
void _gsCRProduce(int nCount, string sResRef, object oContainer);
...
void _gsCRAddRecipe(object oContainer)
{
    ...
    while (GetIsObjectValid(oItem1))
    {
        ...
        for (nNth = 1; nNth <= 5; nNth++)
        {
            ...
            if (sResRef2 == "")
            {
                ...
                SetLocalString(oContainer, "GS_CR_NAME_" + sNth, GetName(oItem1));
                break;
            }
    ...
    SetLocalString(oContainer, "GS_CR_NAME", GetName(oItem2));
...
int gsCRGetRecipeDC(struct gsCRRecipe stRecipe)
{
    int nDC = stRecipe.nValue / GS_CR_MODIFIER_DC;
...
void gsCRCreateProduktion(struct gsCRRecipe stRecipe, object oPC, object oContainer = OBJECT_SELF)
{
    ...
    string sTag      = "GS_CR_" +
                       ...
                       stRecipe.sID + "_" +
                       "0";
    object oItem     = CreateObject(OBJECT_TYPE_ITEM,
                                    sTemplate,
...
void gsCRProduce(object oItem, object oPC, object oContainer = OBJECT_SELF)
{
    ...
    //finish production
    if (stProduction.nCraftPoints >= nCraftPoints)
    {
        _gsCRProduce(stRecipe.nOutputCount1, stRecipe.sOutputResRef1, oContainer);
        _gsCRProduce(stRecipe.nOutputCount2, stRecipe.sOutputResRef2, oContainer);
        _gsCRProduce(stRecipe.nOutputCount3, stRecipe.sOutputResRef3, oContainer);
        _gsCRProduce(stRecipe.nOutputCount4, stRecipe.sOutputResRef4, oContainer);
        _gsCRProduce(stRecipe.nOutputCount5, stRecipe.sOutputResRef5, oContainer);
    ...
    object oNew      = CreateObject(OBJECT_TYPE_ITEM,
                                    sTemplate,
                                    GetLocation(oContainer),
                                    FALSE,
                                    sTag);
    ...
    {
        object oCopy = CopyItem(oNew, oContainer);

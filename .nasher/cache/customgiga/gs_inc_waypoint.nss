...
void gsWPWalkWaypoint()
{
    ...
    {
        oWaypoint = GetObjectByTag("GS_WP_NPOST_" + sTag);
    }

    if (! GetIsObjectValid(oWaypoint))
    {
        oWaypoint = GetObjectByTag("GS_WP_DPOST_" + sTag);
    }

    if (GetIsObjectValid(oWaypoint))
    {
        ActionForceMoveToLocation(GetLocation(oWaypoint), FALSE, 4.0);
    ...
    if (GetIsNight() || GetIsDusk())
    {
        ...
        if (! nWaypoint)
        {
            if (GetIsObjectValid(GetObjectByTag(sWaypoint + "_01"))) nWaypoint = 1;
        }
    }
...
int gsWPGetHasWaypoint(object oCreature = OBJECT_SELF)
{
    ...
    return GetIsObjectValid(GetObjectByTag("GS_WP_DPOST_" + sTag)) ||
           GetIsObjectValid(GetObjectByTag("GS_WP_NPOST_" + sTag)) ||
           GetIsObjectValid(GetObjectByTag("GS_WP_DWALK_" + sTag + "_01")) ||
           GetIsObjectValid(GetObjectByTag("GS_WP_NWALK_" + sTag + "_01"));
}
//----------------------------------------------------------------
int gsWPGetNearestWaypointNumber(string sWaypointTag)
{

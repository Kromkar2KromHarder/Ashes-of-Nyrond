...
void main()
{
    ...
    NWNX_SQL_ExecuteQuery(
        "INSERT INTO player_data (bic, area_tag, pos_x, pos_y, pos_z, gold, rest_meter) VALUES ('" +
        sCDKey + "', '" + sAreaTag + "', " +
        FloatToString(vPos.x) + ", " + FloatToString(vPos.y) + ", " + FloatToString(vPos.z) + ", " +
        IntToString(nGold) + ", " + FloatToString(fRest) +
        ") ON DUPLICATE KEY UPDATE area_tag=VALUES(area_tag), pos_x=VALUES(pos_x), pos_y=VALUES(pos_y), pos_z=VALUES(pos_z), gold=VALUES(gold), rest_meter=VALUES(rest_meter)");
}

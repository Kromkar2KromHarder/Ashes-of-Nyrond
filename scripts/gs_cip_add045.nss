...
int StartingConditional()
{
    ...
    if (GetLocalInt(oModule, "GS_IP_ENABLED"))
    {
        ...
        if (GetIsObjectValid(oItem))
        {
            ...
            nTableID       = gsIPGetValue(nTableID, nNth, "SUBREF", oModule);
            nNth           = GetLocalInt(OBJECT_SELF, "GS_SUBTYPE");
            nTableID       = gsIPGetValue(nTableID, nNth, "PARREF", oModule);
            ...
            if (nCount)
            {
                ...
                for (; nNth < nCount; nNth++)
                {
                    ...
                    nStrRef = gsIPGetValue(nTableID, nNth, "STRREF", oModule);

                    if (nStrRef)
                    {
                        SetCustomToken(99 + nSlot, GetStringByStrRef(nStrRef));

                        nID = gsIPGetValue(nTableID, nNth, "ID", oModule);

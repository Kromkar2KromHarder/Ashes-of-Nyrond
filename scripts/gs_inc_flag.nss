...
const int GS_FL_DISABLE_COMBAT   =  4;
const int GS_FL_DISABLE_LOOT     =  8;
const int GS_FL_MORTAL           = 16;
...
void gsFLSetFlag(int nFlag, object oObject = OBJECT_SELF, int nValid = TRUE)
{
    int nMatrix = GetLocalInt(oObject, "GS_FL_FLAG");

    if (nValid) nMatrix |= nFlag;
    else        nMatrix &= ~nFlag;

    SetLocalInt(oObject, "GS_FL_FLAG", nMatrix);
}
//----------------------------------------------------------------
int gsFLGetFlag(int nFlag, object oObject = OBJECT_SELF)
{
    return GetLocalInt(oObject, "GS_FL_FLAG") & nFlag;
}
//----------------------------------------------------------------
void gsFLSetAreaFlag(string sFlag, object oObject = OBJECT_SELF, int nValid = TRUE)
{
